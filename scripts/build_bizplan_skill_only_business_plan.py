...
OUT = ROOT / "output" / "2026-06-20" / "bizplan-skill-only-business-plan"
SOURCES = OUT / "sources"
...
BUSINESS_CORE = """project:
  name: "PlanCore Ledger"
  document_type: "내부 신사업/자유형식 기반 사업계획서" # [가정]
...
EVIDENCE_LEDGER = [
    ...
    ["EL#04", "PlanCore Ledger 고객군과 가격은 아직 검증되지 않았다", "[확인 필요]", "사용자 인터뷰 전", "시장/모델 Gate"],
...
def executive_summary() -> str:
    rows = [
        ["사업명", "PlanCore Ledger", "[가정] 자유주제 사업계획서"],
    ...
    toc = """
<nav class="toc-map" id="document-toc" aria-label="문서 목차"><span class="label">문서 목차</span><p>요약, 결정, 구조, 리스크, 로드맵, 검증, 최종 권고, 출처로 이동합니다.</p><div class="toc-pills"><a class="toc-pill" href="#summary"><b>1</b>Summary</a><a class="toc-pill" href="#decisions"><b>2</b>Decisions</a><a class="toc-pill" href="#architecture"><b>3</b>Core</a><a class="toc-pill" href="#risks"><b>4</b>Risk</a><a class="toc-pill" href="#roadmap"><b>5</b>Roadmap</a><a class="toc-pill" href="#validation"><b>6</b>Gate</a></div></nav>
"""
    return f"""
<div id="summary"></div>
{toc}
{h2('01', 'Executive Summary · PlanCore Ledger 사업계획서', 'summary', 'bizplan 스킬 내용만으로 작성한 자유주제 사업계획서 초안이다. HTML은 adaptive-html-final 템플릿만 사용했다.')}
<p><strong>PlanCore Ledger</strong>는 사업계획서 작성자가 주장, 수치, 출처, 가정, 평가위원 질문을 한 곳에서 추적하도록 돕는 <span class="hl">사업계획서 증거 원장 SaaS</span>다. 이 문서는 외부 시장조사를 하지 않았으므로 시장·매출·고객 수를 만들지 않는다. 대신 bizplan 스킬의 원칙에 따라 모르는 항목을 <span class="tag">[확인 필요]</span>로 남긴다.</p>
{table(rows, '사업계획서 핵심 요약', ['항목', '내용', '증거 태그'])}
<div class="core-insight"><h3>사업계획서의 상품 정의</h3><p>이 사업의 상품은 문장 자동완성이 아니라 <strong>반론을 견디는 사업 논리</strong>다. 사용자가 입력한 아이디어와 공고문을 바로 미문으로 바꾸지 않고, 인터뷰·리서치·business-core.yaml·원장·평가위원 시뮬레이션을 거쳐 제출 가능한 구조로 만든다.</p></div>
<div class="source-note"><h3>사업계획서 본문 초안 · 문제 인식</h3><p><strong>[가정]</strong> 많은 사업계획서 작성 과정은 공고문, 사용자 인터뷰, 시장 리서치, 특허·논문 근거, 재무 가정, 사업비 산식이 서로 다른 문서에 흩어진 상태로 진행된다. 이때 작성자는 최종 제출 직전에 문장만 다듬지만, 평가자는 문장이 아니라 “이 수치의 출처는 무엇인가”, “이 고객은 실제인가”, “이 성능은 연구 환경인가 제품 환경인가”, “이 예산은 실행계획과 연결되는가”를 묻는다. PlanCore Ledger는 이 질문에 답하기 위해 만들어지는 도구다.</p><p><strong>[확인 필요]</strong> 실제 고객이 얼마나 자주 이런 문제를 겪는지, 재작업 시간이 얼마인지, 탈락 사유 중 출처·숫자 불일치가 어느 비중인지, 컨설턴트와 내부 작성자의 지불 의사가 어느 수준인지는 아직 검증되지 않았다. 따라서 본 초안은 시장 규모나 매출 수치를 제시하지 않고, Gate 1 인터뷰와 Gate 2 리서치를 첫 실행 항목으로 남긴다.</p><h3>사업계획서 본문 초안 · 해결책</h3><p><strong>[사실]</strong> bizplan 스킬은 모든 문서가 하나의 <code>business-core.yaml</code>에서 파생되어야 한다고 규정한다. 또한 핵심 주장은 <code>source-index.xlsx</code>와 <code>evidence-ledger.xlsx</code>로 역추적되어야 하며, 문서 유형이 바뀌어도 TAM/SAM/SOM, 가격, 고객 수, 매출, 비용, 인력, 일정, 사업비는 일관되어야 한다. PlanCore Ledger는 이 규칙을 제품의 데이터 모델로 삼는다.</p><p><strong>[목표]</strong> 사용자는 Claim을 입력하면 태그를 선택하고, 출처 행을 연결하고, 어느 섹션에 반영되는지 지정한다. 시스템은 태그 없는 수치, 출처 없는 핵심 통계, 코어와 문서의 숫자 불일치, 미답변 평가자 질문을 제출 전 blocker로 표시한다. 최종 문서는 생성되지만, 생성은 마지막 단계이며 핵심 가치는 검증 과정이다.</p><h3>사업계획서 본문 초안 · 성장 전략</h3><p><strong>[확인 필요]</strong> 초기 고객 세그먼트는 정부지원사업·R&amp;D 과제를 반복 제출하는 스타트업, 연구소, 산학협력단, 사업계획서 컨설턴트로 가정한다. 그러나 구매자와 사용자는 다를 수 있다. 대표가 비용을 지불하고 실무자가 작성하거나, 컨설턴트가 여러 고객 프로젝트를 관리하거나, 기관이 내부 표준 프로세스로 도입할 수 있다. 각 세그먼트는 인터뷰를 통해 문제 강도, 현재 대안, 예산권자, 도입 장벽을 분리해야 한다.</p><p><strong>[가정]</strong> 첫 MVP는 대형 문서 편집기가 아니라 “증거 원장 + business-core.yaml + 평가위원 scorecard”의 최소 흐름으로 충분하다. 문서 출력 품질은 현재 프로젝트의 HTML 템플릿처럼 나중에 붙일 수 있지만, 출처와 가정이 비어 있으면 어떤 템플릿도 사업계획서를 강하게 만들 수 없다.</p></div>
"""


def decision_cards() -> str:
    rows = [
        ["D1", "문서 유형", "내부 신사업/자유형식으로 시작", "정부지원·R&D·IR·제안서 변환은 코어 잠금 후"],
        ["D2", "포지셔닝", "AI 작성기 아님", "증거 원장 + 코어 컴파일러 + 평가위원 시뮬레이터"],
        ["D3", "수치 정책", "시장/매출 수치 금지", "공식 출처와 인터뷰 전까지 [확인 필요]"],
        ["D4", "MVP 범위", "Claim table → source-index → business-core → scorecard", "문서 편집기는 후순위"],
...
{h2('02', 'Decision Cards · 사업 논리를 잠그는 5개 결정', 'decision', '사업계획서의 전제와 미검증 항목을 분리해 다음 실행의 기준으로 삼는다.')}
...
{h2('03', 'Architecture · business-core.yaml 중심 운영 구조', 'architecture', '하나의 코어에서 여러 문서가 파생되고, 모든 주장은 원장으로 역추적된다.')}
{flow}
{table(rows, 'PlanCore Ledger 시스템 구조', ['계층', '역할', 'bizplan 원천'])}
...
"""
    return f"""
<div id="risks"></div>
{h2('04', 'Risk Matrix · 허위 완성보다 정직한 미완을 선택', 'risk', 'bizplan의 7대 절대 규칙을 사업 리스크 관리 체계로 전환했다.')}
{vt}
...
def roadmap() -> str:
    wg = """
<section class="wg-16" aria-labelledby="wg-16-title"><header class="wg-16-head"><p class="wg-16-kicker">Implementation Plan · Skill-only Business Plan</p><h2 id="wg-16-title" class="wg-16-h">90일 MVP 로드맵</h2><p class="wg-16-lead">문서 편집기보다 먼저 증거 태그·코어·평가 게이트를 검증합니다.</p></header><div class="wg-16-panel"><h3 class="wg-16-h3">마일스톤 타임라인</h3><ol class="wg-16-ms"><li class="wg-16-ms-item wg-16-active"><div class="wg-16-ms-mark"><span class="wg-16-ms-node"></span></div><div class="wg-16-ms-card"><div class="wg-16-ms-top"><span class="wg-16-ms-name">M1 · 고객 인터뷰·문제 검증</span><span class="wg-16-badge wg-16-bd-active">0~30일</span></div><p class="wg-16-ms-desc">작성자·구매자·컨설턴트 10건 이상 인터뷰는 [목표]이며, 실제 수는 [확인 필요]로 둡니다.</p></div></li><li class="wg-16-ms-item"><div class="wg-16-ms-mark"><span class="wg-16-ms-node"></span></div><div class="wg-16-ms-card"><div class="wg-16-ms-top"><span class="wg-16-ms-name">M2 · Evidence Ledger MVP</span><span class="wg-16-badge">31~60일</span></div><p class="wg-16-ms-desc">Claim table, source-index, evidence tag, number-registry를 연결합니다.</p></div></li><li class="wg-16-ms-item"><div class="wg-16-ms-mark"><span class="wg-16-ms-node"></span></div><div class="wg-16-ms-card"><div class="wg-16-ms-top"><span class="wg-16-ms-name">M3 · 문서 변환·평가위원 시뮬레이션</span><span class="wg-16-badge">61~90일</span></div><p class="wg-16-ms-desc">정부지원/R&D/IR/제안서 템플릿 변환과 4역할 scorecard를 만든다.</p></div></li></ol><h3 class="wg-16-h3">데이터 플로우</h3><div class="wg-16-flow" aria-label="PlanCore Ledger 데이터 플로우"><div class="wg-16-fnode">Interview<span class="wg-16-fnode-s">CF/assumption</span></div><div class="wg-16-fnode">Research<span class="wg-16-fnode-s">source-index</span></div><div class="wg-16-fnode wg-16-fnode-good">Core<span class="wg-16-fnode-s">business-core.yaml</span></div><div class="wg-16-fnode wg-16-fnode-hot">Ledger<span class="wg-16-fnode-s">claim/evidence</span></div><div class="wg-16-fnode wg-16-fnode-q">Scorecard<span class="wg-16-fnode-s">4 evaluators</span></div></div><h3 class="wg-16-h3">리스크 평가</h3><div class="wg-16-table-wrap"><div class="table-scroll"><table class="wg-16-table"><caption>로드맵 리스크 — 가능성·영향·완화책</caption><thead><tr><th scope="col">리스크</th><th scope="col">가능성</th><th scope="col">영향</th><th scope="col">완화책</th></tr></thead><tbody><tr><th scope="row">인터뷰 응답 부족</th><td><span class="wg-16-lv wg-16-lv-mid">중</span></td><td><span class="wg-16-lv wg-16-lv-hi">높음</span></td><td>세그먼트별 질문 카드와 follow-up-items 자동화</td></tr><tr><th scope="row">원장 입력 피로</th><td><span class="wg-16-lv wg-16-lv-hi">높음</span></td><td><span class="wg-16-lv wg-16-lv-mid">중</span></td><td>source-index 자동 초안 + 사용자 승인</td></tr><tr><th scope="row">문서 출력 품질 과신</th><td><span class="wg-16-lv wg-16-lv-mid">중</span></td><td><span class="wg-16-lv wg-16-lv-hi">높음</span></td><td>scorecard 85점, 무출처 통계 0건 전까지 제출 금지</td></tr></tbody></table></div></div></div></section>
"""
    return f"""
<div id="roadmap"></div>
{h2('05', 'Priority Roadmap · 문서보다 원장부터 만든다', 'roadmap', '90일 안에 검증해야 할 MVP 순서를 bizplan 게이트에 맞춰 배치했다.')}
{wg}
"""
...
{h2('06', 'Validation Checklist · 완료가 아니라 제출 전 미완 목록', 'check', 'bizplan 기준으로 이 사업계획서가 어디까지 사실이고 어디부터 검증 전인지 분리했다.')}
...
<p>PlanCore Ledger는 바로 매출 예측을 제시할 단계가 아니다. bizplan 스킬 기준으로는 <strong>Gate 1 고객 인터뷰</strong>와 <strong>Gate 2 리서치</strong>가 비어 있으므로, 완성 사업계획서가 아니라 <strong>사업 논리 설계 초안</strong>이다.</p>
<p>다음 작업은 단순한 문서 예쁘게 만들기가 아니다. 실제 고객 인터뷰, source-index, evidence-ledger, market-sizing, financial-model을 채운 뒤 다시 HTML/DOCX/PPTX를 파생해야 한다.</p>
"""
...
def copy_sources() -> dict:
    ...
    (SOURCES / "bizplan-application.json").write_text(json.dumps({
        ...
        "selected_topic": "PlanCore Ledger — 사업계획서 증거 원장 SaaS",
        "document_type": "내부 신사업/자유형식 사업계획서",
        "mode": MODE,
        "profile": PROFILE,
...
def render(integrity: dict) -> str:
    ...
    repl = {
        ...
        "{{TITLE}}": "PlanCore Ledger 사업계획서",
        "{{SUBTITLE}}": "사업계획서의 주장·수치·출처·가정·평가 질문을 하나의 business-core.yaml로 추적하는 증거 원장 SaaS 초안",
        "{{META}}": meta,
    ...
    title = f"PlanCore Ledger 사업계획서 · bizplan skill-only · adaptive-html-final v{version()}"
    description = "bizplan 스킬 내용만으로 만든 자유주제 사업계획서 HTML. adaptive-html-final은 템플릿과 검증에만 사용."
    json_ld = json.dumps({
