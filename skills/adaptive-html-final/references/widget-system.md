...
6. **삽입 위치**: 위젯 블록은 `<main id="main">` 내부의 해당 섹션에 배치하며, 본문 흐름을 끊지 않는 보조 뷰로 쓴다. **헤딩 위계(v5.10.3)**: 자체 `<h2>`를 가진 위젯 골격(01·03·09·19 등)은 독립 섹션으로 두는 것이 기본이고, 기존 h2 섹션 *내부*에 중첩 삽입할 때는 위젯 머리글을 `<h3>`로 강등해 h1→h2→h3 위계를 유지한다.
...
| storm_research | **13 Annotated Flowchart**, 14 Feature Explainer, 18 Ticket Triage Board, 11 Weekly Status, 16 Implementation Plan, 04 Timeline | 모순 흐름·관점 발견·미해결 트리아지·검토 상태·후속 액션·출처 시점 |
| social_trend_dashboard | **11 Weekly Status**, 13 Annotated Flowchart, 18 Ticket Triage Board, 14 Feature Explainer, 02 Visual Design Directions | 수집 상태·신호 흐름·미해결 트리아지·기능 설명·시안 비교 |
| strategy_os | **11 Weekly Status**, 13 Annotated Flowchart, 16 Implementation Plan, 18 Ticket Triage Board, 14 Feature Explainer, 02 Visual Design Directions | 운영 상태·신호 흐름·실행 계획·부서 트리아지·기능 설명·시안 비교 |
...
>
> 위젯 적용 갤러리: `examples/`(21모드 참조 예제) — 모드별 wg 사용처는 SKILL.md §0.6 권장 행과 1:1.
