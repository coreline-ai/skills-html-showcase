...
> 도입 이력: 이 라이브러리는 v4.5.0에서 본문 삽입형 HTML 다이어그램으로 편입되었고, 현행 v5.10.5 기준은 `vt-` 21종 + 17모드 §0.6 매핑이다.
...
- 현행 17모드 참조 예제: **`skills/adaptive-html-final/examples/`** — v5.10.5 스킬 자산 기준의 17모드 레퍼런스이며, §0.6의 1순위 vt 계약과 8테마/무JS 검증을 통과해야 한다.
