...
- **mode 20 `social_trend_dashboard` 병합(버전 업 없음)**: 무JS 소셜 트렌드 대시보드 모드 — records 8필드 스키마·URL dedupe/append·지표 정직성(미확인=0/unknown, 추정 금지)·읽기전용 수집·무JS 차트(정적 .tbl + comparison-cards 다운컨버트) 계약 + custom gate 5종(record_schema·url_dedupe·metric_honesty·read_only·no_js_chart). primary_vt `comparison-cards` + wg-11/13/18/14/02 **재사용**, `page` 폭 → **신규 위젯/코어 CSS 없음 → 코어 해시 `a64604d0` 불변**. 19→20 모드, examples 19→20, layouts 19→20, **거버넌스 199→210**(+11). v5.10.6 additive 병합.
- **mode 21 `strategy_os` 병합(버전 업 없음)**: 기술 신호를 전사 행동으로 바꾸는 무JS 전략 운영체계 대시보드 — 사용자가 원본 JS 대시보드(`company_ai_strategy_dashboard.html`)를 제공해 차단 해제. North Star·신호 5종·10부서 주관/협업/참고·실행 워크플로·의사결정 스코어카드·판단 4지선다·운영 리듬 계약 + custom gate 5종(north_star·signal_scenarios·department_roles·decision_framework·**no_js_switcher**). 원본 JS 시나리오 시뮬레이터를 comparison-cards/process-swimlane **무-JS로 다운컨버트**(onclick/data-scenario 금지 게이트). primary_vt `process-swimlane` + wg 재사용, `page` 폭 → **코어 해시 `a64604d0` 불변**. 20→21 모드, examples 20→21, **거버넌스 216→227**(+11). v5.10.6 additive.
- **manual_analysis(mode 8) operator 보강**: 운영자 매뉴얼·증거원장·제출본 검증 흐름을 신규 모드 대신 기존 manual_analysis로 흡수(설계 결정). required_block +3(source_hierarchy·workflow_inventory·handoff_boundary) + `manual_capture_required_gate`(완료-주장 시 CAPTURE_REQUIRED/UNKNOWN/검수 정직 표기 강제 — 과장 차단). 코어 무변경, **거버넌스 210→213**(+3). 기존 16_manual 예제 무회귀.
- **기존 모드 보강 3종(B)**: ① `beginner_html`(mode 12) — `beginner_richness_gate`(입문 출력에 비유·함정·해결 3요소 강제, universal) + required_block `deep_dive_walkthrough`, **거버넌스 213→216**(+3). ② `blog_publication_pack` — blog_writer(11)/platform_blog(2)/seo_dashboard(3)에 발행 패키지 required_blocks(title_variants·meta_json·publish_checklist·final_assets·platform_notes) 생성 가이드 추가(gate는 기존 예제 false-positive 위험으로 제외). ③ `skill_archive_migration` — skill_audit(1)에 `archive_migration_ledger` required_block 추가. ②③은 코어/게이트/예제 무변경(registry 메타데이터만).
