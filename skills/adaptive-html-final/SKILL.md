...
| social_trend_dashboard | social-trend-dashboard.html | comparison-cards, timeline, risk-matrix, quality-gate | wg-11, wg-13, wg-18, wg-14, wg-02 |
| strategy_os | strategy-os-dashboard.html | process-swimlane, comparison-cards, risk-matrix, quality-gate, timeline, hero-map | wg-11, wg-13, wg-16, wg-18, wg-14, wg-02 |
...
14. 신규 산출물 완료 전 390/1280px 렌더 검증을 외부 캡쳐 단계에서 수행하고 `sources/render-audit.json` + screenshot artifact를 남긴다. `completion_check.py`는 Playwright를 직접 구동하지 않고 해당 증빙만 검사한다. **캡쳐 명령(외부 차수)**: `node scripts/render_audit_fulltest.mjs <output_dir>` 실행 후 `node scripts/micro_layout_audit.mjs <output_dir> --merge` — 후자가 G2(diagram 노드 박스 overlap >4px×4px)·G3(`.try` 중첩 흰 카드 링크 대비 <4.5:1)를 측정해 `render-audit.json`의 `micro_layout.checks`(`node_overlap_ok`·`inner_card_link_contrast_ok`)에 병합한다. 푸터가 필요하면 이 슬롯에 채우고, 불필요하면 비워서 footer 없이 렌더링한다.
