...
s1 = f'''<section id="s1">
  ...
  <p>6개월 동안 주말과 새벽을 갈아 넣었다. 결과물은 살아남지 못했지만, 그 시간이 통째로 손해는 아니었다. 다만 같은 교훈을 더 싸게 배울 수 있었다는 게 아쉬울 뿐이다. 이 글은 그 "더 싼 학습"을 누군가에게 미리 넘겨주려는 시도다. 그리고 솔직히, 글로 정리하는 일은 나 자신을 위한 것이기도 하다. 흩어진 후회를 문장으로 묶어 두지 않으면, 다음 프로젝트에서 같은 실수를 또 "처음 겪는 일"처럼 마주하게 되니까.</p>
</section>'''
...
s3 = f'''<section id="s3">
  ...
  <p>가장 뼈아픈 건 4~5개월이다. 3개월 차 신호(재방문 없음)를 보고도, 나는 "기능을 더 만들면 해결된다"는 익숙한 답으로 도망쳤다. 만드는 일은 즐겁고, 직시하는 일은 불편하기 때문이다. 타임라인을 이렇게 한 줄로 정리해 보니, 가장 중요한 분기점은 6개월 차의 종료 결정이 아니라 3개월 차에 신호를 무시한 그 순간이었다. 그때 멈추거나 방향을 틀었다면, 잃은 건 6개월이 아니라 3개월이었을 것이다.</p>
</section>'''
...
s6 = f'''<section id="s6">
  ...
  <p>경제학에서 매몰비용은 의사결정에서 무시해야 할 비용이라고 배운다. 머리로는 안다. 그런데 그 6개월이 내 주말과 자존심으로 이뤄져 있으면, 무시가 안 된다. "이만큼 했으니 조금만 더"가 반복되면서, 손절선이 계속 뒤로 밀렸다. 게다가 주변에 이미 "그거 만들고 있어"라고 말해 둔 탓에, 접는다는 것이 곧 "안 됐다고 인정하는 일"처럼 느껴졌다. 매몰비용에 사회적 체면까지 얹히면 멈춤은 두 배로 어려워진다.</p>
