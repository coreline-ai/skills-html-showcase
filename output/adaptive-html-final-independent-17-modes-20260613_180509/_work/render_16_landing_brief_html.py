...
<section id="s3" class="value-grid summary-card">
  ...
  <p>가치의 공통 축은 "시간"이다. 흩어진 지식을 찾느라 쓰는 시간 — 묻고, 기다리고, 다시 묻는 — 을 회수하는 것이 Cortex의 ROI다. 도입 후엔 "반복 질문 수"와 "온보딩 자립 시간"으로 효과를 측정하길 권한다. 특히 시니어 엔지니어의 시간은 비싸다. 같은 질문에 매번 답하느라 흐름이 끊기는 비용은 눈에 잘 안 보이지만 누적되면 가장 크다. Cortex의 진짜 가치는 "검색이 좋아진다"가 아니라 "사람이 사람에게 던지던 반복 질문을 시스템이 흡수해 준다"는 데 있다.</p>
</section>'''
...
<section id="s5" class="summary-card">
  ...
  <p>차별점은 마지막 "지식 공백 리포트"다. 단순 검색이 아니라, 무엇이 문서화되지 않았는지를 데이터로 보여줘 지식베이스가 스스로 보강 방향을 가리키게 한다. 검색 도구가 아니라 지식 관리 루프를 만든다. 대부분의 지식베이스는 "검색"까지만 하고 멈춘다. 그러나 검색이 자주 실패하는 주제야말로 가장 문서화가 시급한 곳이다. Cortex는 그 실패를 버리지 않고 모아, "이번 주에 문서화하면 가장 효과 큰 주제"를 알려준다 — 지식의 빈틈이 스스로 드러나게 하는 것이다.</p>
</section>'''
...
<section id="s7" class="summary-card">
  ...
  <p>부적합 사례를 분명히 하는 이유는, 잘못된 도입이 양쪽 모두에 손해이기 때문이다. 문서가 없는 팀은 먼저 기록 문화를 만들어야 하고, 규제 요건이 강한 조직은 셀프호스트 옵션을 먼저 확인해야 한다. 맞는 팀에 들어갈 때 가장 큰 가치가 난다. 그래서 영업도 "모두에게 파는" 대신 적합한 팀을 먼저 가린다.</p>
</section>'''
...
<section id="s9" class="faq summary-card">
  ...
  <p>FAQ에서 반복되는 두 가지 우려는 "권한"과 "신뢰"다. 두 우려 모두 같은 원칙으로 답한다 — 권한은 소스를 그대로 따르고, 신뢰는 출처 링크로 검증 가능하게 한다. 즉 Cortex는 "더 똑똑한 답"을 약속하기보다 "검증할 수 있는 답"을 약속한다. 검증 가능성이 곧 지식베이스의 신뢰이기 때문이다.</p>
</section>'''
