...
<section id="s1" class="summary-card">
  ...
  <p>SEO의 출발은 키워드가 아니라 "검색자가 이루려는 일"이다. 이 포스트는 단순 소개가 아니라 "따라 하면 구축되는" 실무 가이드를 약속하므로, 제목·메타·본문이 그 약속을 일관되게 지켜야 검색·체류 모두 좋아진다. 반대로 키워드만 노리고 약속을 지키지 않으면, 검색 유입은 잠깐 늘어도 즉시 이탈이 쌓여 결국 순위가 내려간다. 그래서 이 대시보드는 "어떻게 상위에 노출될까"보다 "검색자가 원하는 구축 절차를 실제로 줄 수 있는가"를 먼저 점검한다.</p>
</section>'''
