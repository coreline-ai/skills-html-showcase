...
s2 = f'''<section id="s2">
  ...
  <p>지금 돌아보면 그 "좋다"가 함정이었다. 칭찬과 사용은 다른 일이다. 나는 "쓰겠다"는 말과 "써 봤다"는 행동을 구분하지 않았고, 사회적 호응을 시장 신호로 착각했다. 시작의 동기는 진심이었지만, 검증의 기준은 처음부터 허술했다.</p>
</section>'''
...
s4 = f'''<section id="s4">
  ...
  <p>멈춤의 신호는 대개 "더 노력하라"가 아니라 "가설이 틀렸다"고 말한다. 그런데 노력은 내가 통제할 수 있고 가설 수정은 자존심이 상하니, 사람은 자연히 노력 쪽으로 도망친다. 그게 6개월을 끌게 만든 진짜 원인이었다.</p>
</section>'''
...
s8 = f'''<section id="s8">
  ...
  <p>요약하면 "더 빨리 틀리기"다. 6개월에 걸쳐 비싸게 틀리는 대신, 한 달 안에 싸게 틀리고 방향을 바꾸는 것. 사이드 프로젝트의 진짜 자산은 결과물이 아니라 이 학습 속도다.</p>
</section>'''

s9 = f'''<section id="s9">
  {h2(None,"남은 생각","quote")}
  <p>접고 나니 의외로 후회는 없다. 6개월의 시간은 결과물 대신 "판단 근육"을 남겼다. 다음엔 더 빨리 신호를 읽고, 더 일찍 방향을 틀고, 필요하면 더 담담하게 멈출 수 있을 것이다.</p>
  <p>그리고 무엇보다, 끝내는 일을 부끄러워하지 않게 됐다. 잘 끝낸 프로젝트는 실패가 아니라 학습이다. 시작만큼 멈춤도 실력이라는 것 — 그게 6개월이 내게 준 가장 비싼, 그러나 값진 교훈이다.</p>
</section>'''
