...
(async () => {
  ...
  for (const [label, width, height] of [['1280',1280,1600], ['390',390,1600]]) {
    ...
    await page.screenshot({path:`/tmp/ahf-mode05-after-${label}.png`, fullPage:true});
    results[label] = await page.evaluate(() => {
      ...
      const tocLinks = [...document.querySelectorAll('.github-question-toc a,.toc a')];
      const tocTargetsOk = tocLinks.every(a => a.hash && document.querySelector(a.hash));
      return {
  ...
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode05-after-metrics.json', JSON.stringify(results, null, 2));
})();
