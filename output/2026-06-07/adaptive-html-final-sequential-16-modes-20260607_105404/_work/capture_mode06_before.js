...
(async () => {
  ...
  for (const [label, width, height] of [['1280',1280,1600], ['390',390,1600]]) {
    ...
    await page.screenshot({path:`/tmp/ahf-mode06-before-${label}.png`, fullPage:true});
  ...
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode06-before-metrics.json', JSON.stringify(results, null, 2));
})();
