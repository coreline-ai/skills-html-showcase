...
(async () => {
  ...
  for (const [label, width, height] of [['1280',1280,1600], ['390',390,1600]]) {
    ...
    await page.screenshot({path:`/tmp/ahf-mode04-after-${label}.png`, fullPage:true});
    results[label] = await page.evaluate(() => {
      ...
      const directH2IconOrder = h2s.every(h => {
  ...
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode04-after-metrics.json', JSON.stringify(results, null, 2));
})();
