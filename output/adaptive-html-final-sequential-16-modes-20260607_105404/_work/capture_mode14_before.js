...
const htmlPath = path.join(root, 'pages', '14_case_study_search_index_lag_postmortem.html');
...
async function collect(page, width) {
  return await page.evaluate((width) => {
    ...
    return {
      ...
      timelineCards: document.querySelectorAll('.timeline-card li,.wg-12-tl-item,.tl-item').length,
      situation: !!document.querySelector('.summary-card'),
      timeline: !!document.querySelector('section.timeline'),
      decisions: !!document.querySelector('section.decisions'),
      results: !!document.querySelector('section.results'),
      lessons: !!document.querySelector('.try'),
...
(async()=>{
  ...
  for (const [width,suffix] of [[1280,'1280'],[390,'390']]) {
    ...
    await page.screenshot({path:path.join(shotDir,`mode14-case-before-${suffix}.png`), fullPage:true});
    results.push(await collect(page,width));
    await page.close();
  }
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode14-before-metrics.json', JSON.stringify(results,null,2));
