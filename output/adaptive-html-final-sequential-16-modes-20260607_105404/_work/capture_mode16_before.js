...
const htmlPath = path.join(root, 'pages', '16_checklist_feature_flag_release_safety_playbook.html');
...
async function collect(page, width) {
  return await page.evaluate((width) => {
    ...
    return {
      ...
      useCase: !!document.querySelector('.summary-card'),
      checkGrid: !!document.querySelector('.check-grid'),
      failureModes: !!document.querySelector('.failure-modes'),
      doneCriteria: !!document.querySelector('.try'),
...
(async()=>{
  ...
  for (const [width,suffix] of [[1280,'1280'],[390,'390']]) {
    ...
    await page.screenshot({path:path.join(shotDir,`mode16-checklist-before-${suffix}.png`), fullPage:true});
    results.push(await collect(page,width));
    await page.close();
  }
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode16-before-metrics.json', JSON.stringify(results,null,2));
