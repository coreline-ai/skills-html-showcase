...
(async()=>{
  ...
  for (const [width,suffix] of [[1280,'1280'],[390,'390']]) {
    ...
    await page.screenshot({path:path.join(shotDir,`mode15-landing-after-${suffix}.png`), fullPage:true});
    results.push(await collect(page,width));
    await page.close();
  }
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode15-after-metrics.json', JSON.stringify(results,null,2));
