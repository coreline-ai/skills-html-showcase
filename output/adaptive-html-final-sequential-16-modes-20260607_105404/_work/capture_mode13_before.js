...
const htmlPath = path.join(root, 'pages', '13_comparison_internal_search_architecture_matrix.html');
...
async function collect(page, width) {
  return await page.evaluate((width) => {
    ...
    return {
      ...
      decisionContext: !!document.querySelector('.decision-context'),
      matrix: !!document.querySelector('section.matrix'),
      winners: !!document.querySelector('section.winners'),
      tradeoffs: !!document.querySelector('section.tradeoffs'),
      recommendation: !!document.querySelector('.try'),
...
(async()=>{
  ...
  for (const [width,suffix] of [[1280,'1280'],[390,'390']]) {
    ...
    await page.screenshot({path:path.join(shotDir,`mode13-comparison-before-${suffix}.png`), fullPage:true});
    results.push(await collect(page,width));
    await page.close();
  }
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode13-before-metrics.json', JSON.stringify(results,null,2));
