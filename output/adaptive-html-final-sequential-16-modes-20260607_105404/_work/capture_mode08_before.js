...
async function cap(width,height){
 ...
 const m=await page.evaluate(()=>{
  ...
  return {
   ...
   directSections:direct.length, h2:h2.length, numberedH2:numbered.length, numberedIcon:icon, h2IconComplete: icon===numbered.length,
   ...
   tables:$('table').length, captions:$('caption').length,
   unsafeTables:$('table').filter(t=>!t.closest('.table-scroll')&&!t.classList.contains('mobile-card-table')).length,
 ...
 const screenshot=path.join(outDir,`mode08-expert-before-${width}.png`);
...
(async()=>{
 ...
 fs.writeFileSync('/tmp/ahf-mode08-before-metrics.json',JSON.stringify(runs,null,2));
