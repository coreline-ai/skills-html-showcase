...
(async()=>{
  ...
  for (const [width,suffix] of [[1280,'1280'],[390,'390']]) {
    ...
    await page.screenshot({path:path.join(shotDir,`mode13-comparison-after-${suffix}.png`), fullPage:true});
    results.push(await collect(page,width));
    await page.close();
  }
  await browser.close();
  fs.writeFileSync('/tmp/ahf-mode13-after-metrics.json', JSON.stringify(results,null,2));
