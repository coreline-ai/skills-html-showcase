...
> 버전 변경은 사용자 명시 승인 또는 `dev-plan/release-approval-vX.Y.Z.md`가 있을 때만 허용한다. 전체 수정 표면과 절차는 [버전 릴리스 정합 가이드](docs/adaptive-html-final-version-release-guide.md)와 [버전 표면 전체 파일 감사 기록](docs/adaptive-html-final-version-surface-audit-20260613.md)을 먼저 따른다.
...
3. **현행 설명 표면 동기화** — `manifest.examples.purpose`, `manifest.quality.governance_count`, `README.md`의 “현행 21모드 참조 예제”·“게이트 현황”, `Guide.md`, `AGENTS.md`, `references/*` 중 “현행/현재/스킬 자산 기준” 문구를 최신 버전·최신 게이트 수로 맞춘다. 현재 게이트 수는 manifest의 단일 필드를 기준으로 한다. 과거 CHANGELOG·역사적 기준선 설명은 보존하되 “현행”으로 읽히지 않게 쓴다.
...
여러 트리거가 겹치면 `skills/adaptive-html-final/SKILL.md` §3의 Priority(1=skill_audit … 20=social_trend_dashboard, 21=strategy_os)를 따른다.
...
| `strategy_os` | AI 전략 대시보드, 회사 전략 운영체계, 부서별 관점, 기술 신호 운영, 전략 OS | `.layout-strategy-os` | `strategy-os-dashboard.html` | **process-swimlane** → comparison-cards → risk-matrix → quality-gate → timeline → hero-map | wg-11, wg-13, wg-16, wg-18, wg-14, wg-02 |
...
0. **프로파일 결정 (모드 결정보다 선행, 라우팅의 첫 입력)** — 비주얼 프로파일을 먼저 확정한다. 현행 정본 인자는 `profile=widget|diagram|auto`뿐이며 `trim→lowercase→정규화`한다. 무효·범위 밖 토큰은 **`invalid_profile`로 실패**(조용한 auto 폴백 금지). **인자 미지정 시: 이 `AGENTS.md` 경유 에이전트(Codex/Gemini 등)는 질문 없이 무조건 `auto`** (대화형 1회 질문은 Claude 대화 한정 UX이며 결정론 보장 대상 아님). 프로파일이 §4(4) CSS 번들·삽입 단계(6·7)·§3 결정표 컬럼 사용을 게이트한다(widget=wg 컬럼만/diagram=vt 컬럼만/auto=둘 다). 역사적 style 별칭은 파서 호환 목적 외 문서·manifest·예제 표면에 노출하지 않는다.
...
4. **CSS 인라인 (순서 고정, `base.html` 슬롯 대응)** — 아래 순서로만 인라인한다.
   ...
   - **프로파일별 CSS 번들(§4 Step 0 결정 적용):**
     ...
     - `auto` = **혼합/기본 번들**(widget+diagram 둘 다)이며 **회귀-0 기준선**(Phase -1 baseline 과 SHA256 diff 0). `widget`·`diagram`은 단일 라이브러리 프로파일이고 `auto`는 그 둘을 합친 기본값이다(`auto ≠ diagram`).
...
5. **21모드 라우터 고정.** §3 결정표 외 모드를 만들지 않는다.
...
- **현행 레퍼런스 출력:** `skills/adaptive-html-final/examples/`는 v5.10.6 스킬 자산 기준의 21모드 참조 예제 세트다.
- **역사적 13-topic 기준선:** `output/2026-06-05/adaptive-html-final-13-topics-20260605_083433/`는 v5.2.3 시점 캐노니컬 예시로 보존하되, 최신 21모드 기준선으로 오해하지 않는다.
...
| 21모드 라우팅 상세 | `references/mode-selection.md` |
